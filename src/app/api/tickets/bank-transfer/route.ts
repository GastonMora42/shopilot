...
async function executeWithRetry(fn: { (): Promise<NextResponse<{ success: boolean; ticketId: any; transferTicketId: any; message: string; }> | NextResponse<{ success: boolean; error: string; }>>; (): any; }, maxRetries = 3, delay = 500) {
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      lastError = error;
      // Comprobar si es un error de bloqueo
      if (
        error instanceof Error && error.message.includes('Unable to acquire') ||
        // Para errores de MongoDB con código
        typeof error === 'object' && error !== null && 'code' in error && error.code === 24
      ) {
        console.log(`Reintento ${attempt}/${maxRetries} después de error de bloqueo`);
        await new Promise(resolve => setTimeout(resolve, delay * attempt));
        continue;
      }
      throw error; // Si no es un error de bloqueo, relanzarlo
    }
  }
...
export async function POST(req: Request) {
  return executeWithRetry(async () => {
    ...
    try {
      ...
      // Iniciar transacción con tiempo de espera ampliado
  ...
  }, 3, 1000); // 3 reintentos con 1000ms de espera base
}