...
const calculateTotal = (): number => {
  return selectedTickets.reduce((sum, selected) => {
    const ticket = tickets.find(t => 
      (t._id === selected.ticketId) || (t.id === selected.ticketId)
    );
    return sum + (ticket?.price ?? 0) * selected.quantity;
  }, 0);
};

const handleQuantityChange = (ticket: GeneralTicket, newQuantity: number) => {
  const ticketId = ticket._id || ticket.id; // Fallback a id si _id no existe
  
  console.log('GeneralTicketSelector - handleQuantityChange:', { 
    ticketId,
    newQuantity,
    ticket
  });

  if (!ticketId) return; // Early return si no hay ID

  // Si estamos seleccionando un nuevo ticket, limpiar los otros
  if (newQuantity > 0) {
    selectedTickets.forEach(st => {
      if (st.ticketId !== ticketId && st.quantity > 0) {
        onTicketSelect(st.ticketId, 0);
      }
    });
  }
 ...
 return (
   <div className="space-y-6">
    {tickets
      .filter(ticket => ticket._id || ticket.id) // Solo procesar tickets con ID
      .map(ticket => {
      ...
      >
           <div className="flex justify-between items-start">
             ...
             <div className="flex flex-col items-end space-y-2">
               ...
               <button
                onClick={() => handleQuantityChange(ticket, Math.max(0, currentQuantity - 1))}
                disabled={currentQuantity === 0}
                 ...
                 <button
                onClick={() => handleQuantityChange(ticket, currentQuantity + 1)}
                disabled={
       ...
       >
         ...
         <div className="space-y-2">
           {selectedTickets.map(selected => {
             const ticket = tickets.find(t => t._id === selected.ticketId);
